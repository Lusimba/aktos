...
class ConsumerListAPIViewTest(TestCase):
    ...
    def test_loading_data_to_db(self):
        ...
        self.assertEqual(
            last_entry.previous_jobs_count, last_data["previous_jobs_count"]
        )
        self.assertEqual(last_entry.amount_due, last_data["amount_due"])
        self.assertAlmostEqual(
            last_entry.location.x, last_data["location"][0], places=6
        )
        self.assertAlmostEqual(
            last_entry.location.y, last_data["location"][1], places=6
        )
    ...
    def test_consumer_detail_view(self):
        ...
        self.assertEqual(
            response_properties["previous_jobs_count"], consumer.previous_jobs_count
        )
